...
一次性 RAG 问答。M8 起 token 级流式输出（CPU 跑 4B 不再"干等 30 秒"）。
...
⚠️ `chat` **不**做"续接 session" —— M7.1 实装发现几乎没人用，已 drop。每次启动都是新会话；session_id 仅供 sqlite 主键用。
...
启动 M12 GPUI 桌面 GUI 启动器（7 页 sidebar + 托盘）。需要 `--features gui` 编译：
