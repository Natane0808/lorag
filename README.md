...
>
> 两种使用方式：**桌面 GUI**（双击 exe）/ **CLI**（命令行，CLI 内置 Web UI 浏览器聊天）。
...
| 想要浏览器聊天 / 历史 / 图表 | **CLI 服务模式** | `lorag serve` → 浏览器自动打开 |

所有前端共享同一份数据（`./data/`），任意切换。lorag 本质上是 **CLI**；Web UI 是 CLI 内置的 axum HTTP server 提供的，不是独立的前端产品。
...
- **多轮对话 + RAG**：SQLite 历史 + 检索增强；token 级流式输出
- **可选 rerank**：配 `RERANK_MODEL=` 启用，召回 +15-25%
- **4 层防注入**：sanitize + chunk 边界包裹 + 系统铁律 + recency bias
- **混合检索（opt-in）**：BM25 + 向量 RRF 融合，大文档量时互补
- **内置 Web UI**：`lorag serve` 启动 axum + SolidJS + daisyUI，SSE 流式 + Mermaid 图表
- **系统托盘** / **GPUI 桌面 GUI**：开箱即用面对办公小白
...
# 2. 编译（默认 CPU；有 NVIDIA GPU 加 --features cuda 加速）
cargo build # 默认 CPU（首次几分钟，之后增量 ~30 秒）
cargo build --features cuda # NVIDIA CUDA 加速（首次 5-10 分钟）
...
lorag serve # 内置 Web UI（浏览器聊天，自动打开）
```
...
- [GPUI](https://github.com/zed-industries/zed) + [gpui-component](https://github.com/longbridge/gpui-component) — 桌面 GUI 框架