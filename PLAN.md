...
> - 桌面 GUI 使用 → [doc/gui.md](doc/gui.md)
...
| 系统托盘 | `tray-icon` 0.19 + `image` 0.25 + `windows-sys` 0.59 | `lorag tray` + GUI 托盘共存 |
| 桌面 GUI | `gpui` / `gpui_platform` (zed) + `gpui-component@57a9903f` + `rfd` 0.15 + `tracing-appender` 0.2 | `lorag-gui`（feature flag `gui` 隔离） |
...
│   ├── gui_main.rs             # GUI bin 入口（GPU probe + GPUI bootstrap + tokio runtime + tray）
│   ├── lib.rs                  # 模块声明
...
│   ├── tray.rs                 # 系统托盘核心（GUI 复用 open_browser）
│   ├── server.rs               # axum HTTP server + SSE 流式 API + 嵌入式前端
│   ├── ingest/
│   │   ├── loader.rs           # 按扩展名分派
...
- `llm_generate_stream(params)`：流式版，通过 `mpsc::channel(64)` 桥接 aha `generate_stream`。返回 `Receiver<Result<String>>` 逐 token 消费
...
/// 流式版 llm_complete，通过 mpsc channel 逐 token 输出。
...
/// 构建 chat 多轮对话 preamble（cfg 参数新增，重构）
...
- `sqlite_store.rs`：
  ...
  - `chunks_fts` FTS5 虚拟表（`unicode61` tokenizer，B排序）
  - `search_fts(query, limit)`：把用户问题转为 OR 查询（`build_fts5_query` 提取拉丁/数字 token + 中文单字，OR 连接），B排名
...
系统托盘核心（`lorag tray`）：axum server + 托盘图标常驻，浏览器自动打开。
...
### 4.9 `gui/`（GPUI 桌面启动器，`lorag-gui`）

第四个前端（CLI / server / tray / gui），通过 `gui` feature flag 隔离（默认 OFF，`cargo build` 不拉 GPUI 依赖）。

**目的**：办公小白双击 `lorag-gui.exe` 就能用 —— 7 张页面 sidebar 启动器覆盖"服务启停 / 模型下载 / 文档摄入 / 健康检查 / 日志 / .env 设置 / 关于"全流程；聊天走"打开聊天"按钮 → 浏览器开 `localhost:port`（复用 Web UI）。

**关键约束**：

- 依赖 `tray::open_browser`（跨平台开浏览器，复用 逻辑）、`server::start_with_shutdown`、`aha_provider`、`config`、`store::sqlite_store`，**不**直接 `use aha::*` / `use rig_compat::*` / 碰 `chunker`
...
### 4.10 `server.rs`（axum HTTP server）

axum 0.8 + rust-embed 嵌入前端。路由：

- `POST /api/chat`：SSE 流式多轮对话（复用 `llm_complete_stream`）
...
3. **纯向量检索**：关键词召回相对弱。SQLite FTS5 B混合检索已在 实装（`HYBRID_ENABLED=true` 启用），但小数据集下效果不明显 —— 向量检索已覆盖大部分文档。大文档量（100+ 文件、1000+ chunk）时 B可互补召回精确关键词（人名、日期、编号）。目前默认关闭（opt-in）
4. **PDF 扫描版无效**：`pdf-extract` 只读文本层；扫描版得 OCR（aha 本身有 OCR 模型，未实装）
5. **xlsx 多 sheet 行前缀**：多 sheet 时每行加 `[SheetName]` 前缀（修复），但仍不保留表结构 / 公式
